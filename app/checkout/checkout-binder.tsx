...
export default function ModalScreen() {
    ...
    const loadOngkir = async (alamat_penjual: string, alamat_pembeli: string, weight: number, target_toko: string) => {
        const origin = getKodeWilayah(alamat_penjual)?.kode;
        const destination = getKodeWilayah(alamat_pembeli)?.kode;
        const dataOngkir = await cekOngkir(origin, destination, weight);
        setPengiriman(prev => {
        ...
        if (dataCart && !pilihKurir) {
            ...
            for (const toko of dataCart) {
                const [alamat_toko] = toko.data?.map((item: any) => {
                    return item?.mawam_toko?.mawam_profile?.kode_alamat;
                });
                let berat = 0;
                ...
                loadOngkir(alamat_toko, dataUser?.kode_alamat, berat, target_toko);
                setPengiriman(prev => {
                    const data = {
                        target_toko,
               ...
               id,
                ...
                mawam_produk (
                    *,
                    ...
                    alamat_toko,
                    user_id,
                        mawam_profile:user_id(alamat,kode_alamat)
                    )
                )
            `)
            .in("id", cartIds);
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}>
                                                        ...
                                                        <View style={{ flex: 1 }}>
                                                            ...
                                                            <ThemedText style={{ fontSize: 12, opacity: 0.7 }}>
                                                                Estimasi pengiriman {item.estimated} | {item.type}
                                                            </ThemedText>
                                                        </View>
                                                    </View>
                                                    <View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}>
