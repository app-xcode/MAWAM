...
const CATATAN_STATUS: Record<string, string> = {
  "Diterima agen": "Pesanan telah diterima oleh agen untuk diproses.",
...
export default function UpdateLokasiPengiriman() {
  ...
  const [status, setStatus] = useState(STATUS_PERJALANAN[0]);
  ...
  const styles = StyleSheet.create({
    ...
    hint: { opacity: .65, fontSize: 12, marginTop: 6 },
    map: { height: 250, overflow: "hidden", borderRadius: 10 },
    input: { borderWidth: 1, borderColor: "#8888", borderRadius: 8, padding: 10 },
    note: { minHeight: 88, textAlignVertical: "top" },
    statuses: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
    statusOption: { borderWidth: 1, borderColor: "#8888", borderRadius: 16, paddingHorizontal: 10, paddingVertical: 7 },
    statusSelected: { backgroundColor: accentColor, borderColor: accentColor },
    statusSelectedText: { color: "#fff", fontWeight: "700" },
    button: { marginTop: 18, backgroundColor: accentColor, padding: 13, borderRadius: 9, alignItems: "center" },
  ...
  const tampilLokasi = async (lat: number, lng: number) => {
    ...
    for (const component of result.address_components ?? []) {
      ...
      if (!tempat && (types.includes("premise") || types.includes("point_of_interest") || types.includes("establishment"))) {
    ...
    if (!tempat) {
      ...
      tempat = firstPart.includes("+") ? (addressParts?.[1] ?? "") : firstPart;
    }

    setNamaTempat(tempat);
    if (tempat) setDropPoint([tempat, kotaDitemukan].filter(Boolean).join(", "));
    if (kotaDitemukan) setKota(kotaDitemukan);
    setPointAlamat([tempat, kotaDitemukan].filter(Boolean).join(", "));
    setPopup({ id: result.place_id, name: tempat, code: result.formatted_address });
  };
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: delivery, error: deliveryError }, { data: locations, error: locationsError }] = await Promise.all([
      supabase
        ...
        .select("id, order_id, courier_name, courier_code, tracking_number, status, petugas_id, mawam_orders(invoice)")
        .eq("id", pengirimanId)
        .maybeSingle(),
      supabase
    ...
    if (last?.latitude != null && last?.longitude != null) {
      setCoordinates({ latitude: Number(last.latitude), longitude: Number(last.longitude) });
    }
    setLoading(false);
  }, [pengirimanId, user]);
  ...
  if (loading) {
    return <View style={styles.center}><ActivityIndicator size="large" color={iconColor} /><ThemedText>Memuat pengiriman...</ThemedText></View>;
  }
      ...
      <ThemedView style={styles.card}>
        ...
        <View style={styles.map}>
          <MapPicker
            ...
            popup={popup}
            onLocationChange={(latitude, longitude) => setCoordinates({ latitude, longitude })}
          />
        </View>
        ...
        <View style={styles.statuses}>
          {STATUS_PERJALANAN.map((item) => (
            <TouchableOpacity
              key={item}
              onPress={() => setStatus(item)}
              style={[styles.statusOption, status === item && styles.statusSelected]}
            >
              <ThemedText style={status === item ? styles.statusSelectedText : undefined}>{item}</ThemedText>
            </TouchableOpacity>
          ))}
        </View>

        <ThemedText style={styles.label}>Catatan</ThemedText>
        <ThemedInput value={catatan} onChangeText={setCatatan} style={[styles.input, styles.note]} multiline placeholder="Pesanan telah diterima oleh agen untuk diproses." placeholderTextColor="#888" />

        <TouchableOpacity style={styles.button} onPress={() => void save()} disabled={saving}>
          <ThemedText style={styles.buttonText}>{saving ? "Menyimpan..." : "UPDATE LOKASI"}</ThemedText>
        </TouchableOpacity>
      </ThemedView>
    </ScrollView>
  </>;
